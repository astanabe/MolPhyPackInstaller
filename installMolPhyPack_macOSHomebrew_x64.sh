...
if ! test -e .java; then
wget -c https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.20.1%2B1/OpenJDK11U-jre_x64_mac_hotspot_11.0.20.1_1.tar.gz || exit $?
tar -xzf OpenJDK11U-jre_x64_mac_hotspot_11.0.20.1_1.tar.gz || exit $?
mkdir -p $PREFIX/share/molphypack || exit $?
mv jdk-11.0.20.1+1-jre/Contents/Home/* $PREFIX/share/molphypack/ || exit $?
rm -rf jdk-11.0.20.1+1-jre OpenJDK11U-jre_x64_mac_hotspot_11.0.20.1_1.tar.gz || exit $?
touch .java || exit $?
fi
...
gtar -xzf EMBOSS-6.6.0.tar.gz || exit $?
...
gtar -xzf mafft-7.505-without-extensions-src.tgz || exit $?
...
gtar -xzf primer3-2.6.1.tar.gz || exit $?
cd primer3-2.6.1/src || exit $?
perl -i -npe 's/^O_OPTS *= */$& -mtune=native /' Makefile || exit $?
gmake -j4 || exit $?
mkdir -p $PREFIX/bin || exit $?
mv long_seq_tm_test ntdpal ntthal oligotm primer3_core $PREFIX/bin/ || exit $?
cd ../.. || exit $?
...
perl -i -npe 's/^CFLAGS *= */$& -O3 -mtune=native -fcommon /' Makefile || exit $?
gmake -j4 || exit $?
cp mrbayes5d $PREFIX/bin/mrbayes5d-mpi || exit $?
cd .. || exit $?
...
gtar -xzf FigTree_v1.4.4.tgz || exit $?
cd FigTree_v1.4.4/lib || exit $?
mkdir -p $PREFIX/share/figtree || exit $?
...
gtar -xzf ../Tracer_v1.7.2.tgz || exit $?
cd lib || exit $?
mkdir -p $PREFIX/share/tracer || exit $?
mv *.jar $PREFIX/share/tracer/ || exit $?
cd ../bin || exit $?
...
mkdir -p $PREFIX/share/kakusan4 || exit $?
mv kakusan4.pl baseml $PREFIX/share/kakusan4/ || exit $?
echo '#!/bin/sh' > kakusan4 || exit $?
echo "perl $PREFIX/share/kakusan4/kakusan4.pl \$*" >> kakusan4 || exit $?
chmod 755 kakusan4 || exit $?
mv kakusan4 $PREFIX/bin/ || exit $?
cd .. || exit $?
...
perl -i -npe 's/^CFLAGS *= */$&-mtune=native -fcommon /;s/ -frerun-loop-opt//;s/ -static-libgcc//' Makefile.UNIX || exit $?
gmake -j4 -f Makefile.UNIX || exit $?
mkdir -p $PREFIX/share/aminosan || exit $?
mv aminosan.pl codeml $PREFIX/share/aminosan/ || exit $?
echo '#!/bin/sh' > aminosan || exit $?
echo "perl $PREFIX/share/aminosan/aminosan.pl \$*" >> aminosan || exit $?
chmod 755 aminosan || exit $?
mv aminosan $PREFIX/bin/ || exit $?
cd .. || exit $?
