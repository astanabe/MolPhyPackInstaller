sudo yum install gcc gcc-c++ make wget tar gzip bzip2 xz unzip coreutils perl ca-certificates ncurses ncurses-devel readline readline-devel openmpi openmpi-devel || exit $?
